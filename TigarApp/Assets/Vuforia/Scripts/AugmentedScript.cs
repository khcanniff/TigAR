...
public class AugmentedScript : MonoBehaviour
{
    ...
    {
					debugText.text = "AR script,GetCoordinates";
        ...
        while (true)
        {
            ...
            if (Input.location.status == LocationServiceStatus.Failed)
            {
                print("Unable to determine device location");
					debugText.text = "AR script, Geo location LocationServiceStatus.Failed";
                yield break;
            }
            else
            {
                // Access granted and location value could be retrieved\
						debugText.text = "AR script, Geo location enabled";
                print("Location: " + Input.location.lastData.latitude + " " + Input.location.lastData.longitude + " " + Input.location.lastData.altitude + " " + Input.location.lastData.horizontalAccuracy + " " + Input.location.lastData.timestamp);
 	debugText.text = "AR script Location: " + Input.location.lastData.latitude + " " + Input.location.lastData.longitude + " " + Input.location.lastData.altitude + " " + Input.location.lastData.horizontalAccuracy + " " + Input.location.lastData.timestamp;
 ...
 debugText.text = "AR script, start";
    }
    ...
    void Update()
    {
        ...
        transform.eulerAngles += new Vector3(0, 1f, 0);
						//debugText.text = "AR script, update";
        distance_gameobject.text = "Distance from 3D model: " + distance;
    }
}